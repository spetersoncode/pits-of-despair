...
public class GoldPlacer
{
	...
	/// <summary>
	/// Distributes gold across the dungeon floor using floor-depth based formulas.
	/// Gold pile sizes scale exponentially with floor depth.
	/// </summary>
	/// <param name="regions">All regions on the floor</param>
	/// <param name="regionSpawnData">Spawn data per region</param>
	/// <param name="floorDepth">Current floor depth (affects pile sizes and total gold)</param>
		...
		HashSet<Vector2I> occupiedPositions)
	{
		...
		// Get pile count from formula (7-12 piles, relatively constant)
		var (minPiles, maxPiles) = GoldFormula.GetPileCountRange();
		int targetPiles = _rng.RandiRange(minPiles, maxPiles);

		// Get pile size range from formula (scales with floor depth)
		var (minPileSize, maxPileSize) = GoldFormula.GetPileSizeRange(floorDepth, targetPiles);
		...
		goldPlaced += PlaceScatteredGold(regions, scatteredPiles,
			minPileSize, maxPileSize, occupiedPositions);

		int totalBudget = GoldFormula.GetTotalGoldBudget(floorDepth);
		GD.Print($"[GoldPlacer] Placed {goldPlaced} gold in {targetPiles} piles (budget {totalBudget}, floor {floorDepth}, avg pile ~{totalBudget / targetPiles})");
		return goldPlaced;
	}

	/// <summary>
	/// Places gold piles scattered throughout regions.
	/// </summary>
